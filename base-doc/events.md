...
Base record change event. This event is triggered when a subscribed multi-dimensional table record changes. To understand the configuration process and usage scenarios for event subscriptions, refer to [Event Overview](https://open.larkoffice.com/document/ukTMukTMukTM/uUTNz4SN1MjL1UzM). {Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=drive&version=v1&resource=file&event=bitable_record_changed)

**Notice**：- A change in the value of a formula field in a Base table does not trigger an event.
- The value of the formula field is not included in the event body when the Base table record is changed.
- [File edited](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/event/file-edited) event is also triggered when a subscribed Base app record changes.

## Note

If the application subscribes to the event using the application identity (`tenant_access_token`), it is necessary to request permissions for both the application and user identities to receive the event. In other words, to subscribe to this event, you need to enable both the **application identity** and **user identity** permissions for `bitable:app` or `drive:drive` in the Developer console.
## Prerequisite

Before adding this event, you need to ensure that you have called the [Subscribe to Docs events](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file/subscribe) API.
...
Required scopes<br>**To subscribe to this event, enable any of the listed scopes.**<br>Enable any scope from the list | View, comment, edit and manage Base(bitable:app)<br>View, comment, edit, and manage all files in My Space(drive:drive)
...
Push Mode | [Webhook](https://open.larkoffice.com/document/ukTMukTMukTM/uUTNz4SN1MjL1UzM)
...
user_id | string | user's user id<br>**Required field scopes**:<br>Obtain user ID(contact:user.employee_id:readonly)
...
action | string | The change type. Can be:<br>- record_added：add a new record<br>- record_deleted：delete a record<br>- record_edited：edit a record
...
field_value | string | The field value after change occurs. The field is a JSON serialised string, see [data-structure](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/bitable/development-guide/bitable-structure) for the structure before serialisation.
...
update_time | int | The change time. The format is timestamp and the units are in seconds.
...
For event subscription process, refer to:[Event Subscription overview](https://open.larkoffice.com/document/ukTMukTMukTM/uUTNz4SN1MjL1UzM)，Guide for beginners:[Tutorial](https://open.larkoffice.com/document/uAjLw4CM/uMzNwEjLzcDMx4yM3ATM/develop-an-echo-bot/introduction)
...
This event is triggered when a subscribed Base app field changes. To understand the configuration process and usage scenarios for event subscriptions, refer to [Event Overview](https://open.larkoffice.com/document/ukTMukTMukTM/uUTNz4SN1MjL1UzM).{Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=drive&version=v1&resource=file&event=bitable_field_changed)

**Notice**：[File edited](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/event/file-edited) event is also triggered when a subscribed Base app field changes.
...
user_id | string | user's user id<br>**Required field scopes**:<br>Obtain user ID(contact:user.employee_id:readonly)
open_id | string | User's open_id
action_list | bitable.table.field_action\[\] | Field change type. The enumeration values are:<br>- `field_added`: Field added<br>- `field_edited`: Field edited<br>- `field_deleted`: Field deleted
...
user_id | string | User ID
open_id | string | Open ID
update_time | int | Field change time
